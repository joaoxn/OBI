// https://vjudge.net/problem/AtCoder-dp_e
// https://vjudge.net/contest/807537#problem/E

// ? Knapsack 2 with printing taken objects

#include <bits/stdc++.h>
using namespace std;

#define INF LLONG_MAX
typedef long long ll;

ll n, W, MAXV;
vector<ll> w;
vector<ll> v;
vector<vector<ll>> dp;
vector<vector<ll>> trace;

int main() {
    ios_base::sync_with_stdio(0);
    cin.tie(0);

    cin >> n >> W;
    w.resize(n); v.resize(n);
    MAXV = 0;
    for (int i = 0; i < n; i++) {
        cin >> w[i] >> v[i];
        MAXV += v[i];
    }
    dp.resize(n, vector<ll>(MAXV+1, INF));
    trace.resize(n, vector<ll>(MAXV+1, -1));

    dp[0][0] = 0;
    trace[0][0] = 0;
    dp[0][v[0]] = w[0];
    trace[0][v[0]] = v[0];

    for (int i = 1; i < n; i++) {
        for (int val = 0; val < v[i]; val++) {
            dp[i][val] = dp[i-1][val];
            trace[i][val] = val;
        }
        for (int val = v[i]; val <= MAXV; val++) {
            if (dp[i-1][val-v[i]] == INF) { 
                dp[i][val] = dp[i-1][val];
                trace[i][val] = val;
            } else {
                ll take = dp[i-1][val-v[i]]+w[i];
                ll noTake = dp[i-1][val];
                dp[i][val] = min(take,noTake);
                
                if (take < noTake) {
                    trace[i][val] = val-v[i];
                } else {
                    trace[i][val] = val;
                }

            }
        }
    }
    vector<int> path;
    for (int j = MAXV; j >= 0; j--) {
        if (dp[n-1][j] <= W) {
            cout << j << '\n';
            
            int i=n-1, val=j;
            do {
                int lastVal = val;
                val = trace[i][val];
                if (lastVal != val) {
                    path.push_back(i);
                }
            } while (--i >= 0 && trace[i][val] != -1);
            if (val > 0) path.push_back(0);
            break;
        }
    }

    reverse(path.begin(),path.end());
    for (int x : path) cout << x << ' ';
    cout << '\n';

    // for (int i = 0; i < n; i++) {
    //     for (int j = 0; j <= MAXV; j++) {
    //         if (trace[i][j] == -1) cout << 'I' << ' ';
    //         else cout << trace[i][j] << ' ';
    //     }
    //     cout << '\n';
    // }
}

/*

dp[i][w] = max(dp[i-1][w],dp[i-1][w-wi]+vi)

dp[i][v] = min(dp[i-1][v],dp[i-1][v-vi]+wi)

[0] = w=3,v=30
[1] = w=4,v=50
[2] = w=5,v=60

dp[i][w] = max(dp[i-1][w], dp[i-1][w-wi]+vi)
if (noTake >(?=) take)
    dp[i][w] = dp[i-1][w]
    trace[i][w] = w

for (i = [0,n-1])
    for (w = [W,0])
        dp[w] = max(dp[w], dp[w-wi]+vi)

*/